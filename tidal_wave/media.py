...
class Album:
  ...
  def get_items(self, session: Session):
    album_items: AlbumsItemsResponseJSON = \
      request_album_items(session=session, identifier=self.album_id)
  ...
  def get_metadata(self, session: Session):
    self.metadata: AlbumsEndpointResponseJSON = \
      request_albums(session=session, identifier=self.album_id)
  def get_review(self, session: Session):
    if self.album_dir is None:
      self.set_dir(out_dir=out_dir)
    self.album_review: Optional[AlbumsReviewResponseJSON] = \
      request_album_review(session=session, identifier=self.album_id)
    if self.album_review is not None:
      (self.album_dir / "AlbumReview.json").write_text(self.album_review.to_json())
  def set_dir(self, out_dir: Path):
    artist_substring: str = self.metadata.artist.name
    album_substring: str = f"{self.metadata.name} " \
  ...
  def save_cover_image(self, session: Session, out_dir: Path):
    ...
    if not self.cover_path.exists():
      download_cover_image(
        session=session, cover_uuid=self.metadata.cover, output_dir=self.album_dir
      )
    else:
...
class Track:
  ...
  def set_album_dir(self, out_dir: Path):
    ...
    album_substring: str = f"{self.album.name} " \
  ...
  def set_filename(self, audio_format: AudioFormat, out_dir: Path):
    _track_part: str = \
      f"{self.metadata.track_number:02d} - {self.metadata.name}"
  ...
  def download(self, session: Session, out_dir: Path) -> Optional[Path]:
    ...
    self.download_params = \
      {"deviceType": None, "locale": None, "countryCode": None}
    if self.album.number_of_volumes > 1:
      self.outfile: Path = self.album_dir \
        / f"Volume {self.metadata.volume_number}" \
        / self.filename
    else:
    ...
    with NamedTemporaryFile() as ntf:
      for u in urls:
        download_request = session.prepare_request(Request(
          "GET", url=u, headers=self.download_headers, params=self.download_params
        ))
      ...
      if self.codec == "flac":
        ...
        ffmpeg\
          .input(ntf.name, hide_banner=None, y=None)\
          .output(str(self.outfile.absolute()), acodec='copy', loglevel='quiet')\
          .run()
      elif self.codec == "m4a":
  ...
  def craft_tags(self):
    ...
    tags[tag_map["album_artist"]] = \
      ";".join((a.name for a in self.album.artists))
    ...
    tags[tag_map["artist"]] = \
      ";".join((a.name for a in self.metadata.artists))
    tags[tag_map["artists"]] = \
      [a.name for a in self.metadata.artists]
    ...
    if self.codec == "flac":
      ...
      tags["trkn"] = [
        (self.metadata.track_number, self.album.number_of_tracks)
      ]
      tags["disk"] = [
        (self.metadata.volume_number, self.album.number_of_volumes)
      ]
  ...
  def dump(self, fp=sys.stdout):
    json.dump(
      {self.metadata.track_number: str(self.outfile.absolute())},
      fp
    )
