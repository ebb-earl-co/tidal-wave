...
def app(
  tidal_url:        Annotated[str, typer.Argument(help="The Tidal track or album to download")],
  audio_format:     Annotated[AudioFormat, typer.Option(case_sensitive=False)] = AudioFormat.lossless.value,
  output_directory: Annotated[Path, typer.Argument(help=od_help)] = user_music_path(),
  loglevel:         Annotated[LogLevel, typer.Option(case_sensitive=False)] = LogLevel.info.value
):
  logging.basicConfig(
  ...
  tidal_resource: Optional[Union[TidalResource, TidalTrack]] = \
