...
def load_token_from_disk(
  ...
  decoded_token_file_contents: str = \
    base64.b64decode(token_file_contents).decode("utf-8")
...
def validate_token(
  ...
  with requests.get(
    url=f"{TIDAL_API_URL}/sessions", headers=auth_headers
  ) as r:
    try:
      r.raise_for_status()
...
def login_android(
  ...
  if s is None:
    ...
    logger.debug(
      f"Writing this bearer token to '{str(token_path.absolute())}'"
    )
    token_path.write_bytes(
      base64.b64encode(bytes(json.dumps(to_write), "UTF-8"))
    )
  return s
...
def login(
  ...
  if audio_format == AudioFormat.sony_360_reality_audio:
    ...
    while _input not in options:
      _input = input("For which of Android [a], Windows [w], or MacOS [m] would you like to provide an API token? ").lower()
    else:
