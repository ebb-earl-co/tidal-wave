...
TOKEN_DIR_PATH = user_config_path(
  appname=PROJECT_NAME, appauthor=PROJECT_AUTHOR
)
OAUTH2_URL: str = "https://auth.tidal.com/v1/oauth2"
OAUTH2_HEADERS: Dict[str, str] = {
  "User-Agent": "TIDAL_ANDROID/2.38.0", "Accept": "application/json"
}
...
class DeviceAuthorizationEndpointResponseJSON(
  dataclass_wizard.JSONSerializable
):
...
class BearerToken:
  ...
  def __post_init__(self):
    self.client_id, self.client_secret = (
      TidalOauth().client_id, TidalOauth().client_secret
    )
  ...
  def load(cls, p: Path = TOKEN_DIR_PATH / "fire_tv-tidal.token") -> "BearerToken":
    ...
    try:
      ...
      raise TokenException(
        f"File '{str(p.absolute())}' does not exist"
      )
    ...
    data_args = (
      data.get(a) for a in (
        "access_token", "client_name", "expiration",
        "refresh_token", "user_id", "user_name"
      )
    )
  ...
  def refresh(self):
    ...
    with requests.post(
      ...
      if token_json.get("clientName", token_json.get("client_name")) is not None:
        self.client_name = token_json.get("clientName", token_json.get("client_name"))
      ...
      if token_json.get("userName", token_json.get("user_name")) is not None:
        self.user_name = token_json.get("userName", token_json.get("user_nname"))
...
class TidalOauth:
  ...
  def post_device_authorization(
    ...
    _data: Dict[str, str] = {
      "client_id": self.client_id, "scope": "r_usr+w_usr+w_sub"
    }
    with requests.post(url=_url, data=_data, headers=headers) as resp:
      ...
      daerj = DeviceAuthorizationEndpointResponseJSON.from_dict(
          resp.json()
      )

    self.device_authorization = daerj
    self.verification_url: str = \
      f"http://{daerj.verification_uri_complete}"
    ...
    self.verification_expiration: datetime = \
      parsedate_to_datetime(resp.headers.get("Date")) + \
