...
class TracksEndpointResponseJSON(dataclass_wizard.JSONWizard):
  ...
  def __post_init__(self):
    self.name: str = \
      ...
      if self.version is None else \
      f"{self.title.replace('/', '_').replace('|', '_')} ({self.version})"
...
class AlbumsReviewResponseJSON(dataclass_wizard.JSONWizard):
  ...
  /albums/<ID>/review."""
  source: str
  last_updated: Annotated[datetime, dataclass_wizard.Pattern("%Y-%m-%dT%H:%M:%S.%f%z")]
...
class VideosEndpointResponseJSON(dataclass_wizard.JSONWizard):
  ...
  volume_number: int
  track_number: int
  release_date: Annotated[datetime, dataclass_wizard.Pattern("%Y-%m-%dT%H:%M:%S.%f%z")]
  ...
  dj_ready: bool
  stem_ready: bool
  stream_start_date: Annotated[datetime, dataclass_wizard.Pattern("%Y-%m-%dT%H:%M:%S.%f%z")]
...
class VideosContributorsResponseJSON(dataclass_wizard.JSONWizard):
  ...
  def __post_init__(self):
    ...
    self.film_director: Optional[Tuple[str]] = self.get_contributors("Film Director")
    self.film_producer: Optional[Tuple[str]] = self.get_contributors("Film Producer")
    self.lyricist: Optional[Tuple[str]] = self.get_contributors("Lyricist")
    self.mastering_engineer: Optional[Tuple[str]] = self.get_contributors("Mastering Engineer")
    self.producer: Optional[Tuple[str]] = self.get_contributors("Producer")
    self.video_producer: Optional[Tuple[str]] = self.get_contributors("Video Producer")
