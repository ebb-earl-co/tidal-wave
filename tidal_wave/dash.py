...
class XMLDASHManifest:
  ...
  def __post_init__(self):
    ...
    self.audio_sampling_rate = int(self.audio_sampling_rate) \
      if self.audio_sampling_rate is not None else None
    self.timescale = int(self.timescale) \
      if self.timescale is not None else None
    self.startNumber = int(self.start_number) \
      if self.start_number is not None else None
  ...
  def build_urls(self, session: Session) -> Optional[List[str]]:
    ...
    urls_list: List[str] = \
      [self.initialization] + [sub_number(i) for i in number_range]
